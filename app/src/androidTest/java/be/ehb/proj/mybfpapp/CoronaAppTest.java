...
public class CoronaAppTest {
    ...
    public void useAppContext() {
        ...
        assertEquals("be.ehb.proj.mybfpapp", appContext.getPackageName());
    }
    @Test
    ...
    public void shouldWriteSomeTextInTextView(){
        ...
        /*Espresso.onView(withId(R.id.txt_result_accurate)).perform(typeText("Test writing to TextView"));*/
    }
    @Test
    ...
    public void shouldReturnUserRisk0(){
        float expected =0.0f;
        User user = new User(0,"0",1,0.0f);
       onView(withId(R.id.btn_saveSymptoms)).perform(click());
       float actual = user.getUser_risk();
       assertEquals(expected,actual,0.1f);
    }
    @Test
    public void submitATestResult100(){
        float expected =100;
        User user = new User(7,"0",1,100);
        onView(withId(R.id.btn_saveSymptoms)).perform(click());
